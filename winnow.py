...
# Winnow algorithm
def winnow(digit, img, labl, nEpoch=20, learnRate=1.000001):
    # Input & Output formatting
    x_bias = np.ones((img.shape[0],1)) # bias term (nSamplesx1): pass the input value 1
    x = np.hstack((np.round(img / 255.0), x_bias)) # feature space (nSamplesx785): 0 or 1
    y = 1*(labl==digit) + -1*(labl!=digit) # true label (nSamplesx1): -1 or 1
    
    # Initialize weights
    w = np.ones((x.shape[1],1)) # (785x1)
    #print 'w.shape:',w.shape
    
    # Learn and update weights (Gradient descent)
    for iter in xrange(nEpoch):
        for k in range(x.shape[0]): # (nSamples)
            y_hat = np.sign(np.dot(x[k,:], w)) # (1x785)x(785x1)
            #print 'y_hat.shape:',y_hat.shape
            if (y[k] > 0) and (y_hat < 0):
                updateFactor = (1*(x[k,:].T==0) + learnRate*(x[k,:].T!=0)).reshape(785,1)
                w = np.multiply(w, updateFactor)
            elif (y[k] < 0) and (y_hat > 0):
                updateFactor = (1*(x[k,:].T==0) + 1/(learnRate)*(x[k,:].T!=0)).reshape(785,1)
                w = np.multiply(w, updateFactor)
            #print 'updateFactor.shape:',updateFactor.shape,',w.shape:',w.shape
    return w # (nSamplesx1)
...
if __name__ == '__main__':
    ...
    #nTrainToUse = 1000
    #nEpoch = 20
    #learnRate = 1.000001 (1.000001, 1.00001, 1.0001, 1.001, 1.01, 1.1)
    #pathToDataFolder = 'data'
    
    # Train 10 Winnows for all digits (0~9)
    #print '1. Train Winnows for all digits (0~9).'
    img, labl = load_data("training", pathToDataFolder, nTrainToUse)
    weights = np.empty((img.shape[1]+1,0), float) # (785x0)
    for digit in range(10):
        weightForOneDigit = winnow(digit, img, labl, nEpoch, learnRate) # (785x1)
        weights = np.append(weights, weightForOneDigit, axis=1) # (785x10)

    # Test the Winnows with training set
    #print '\n2. Test the Winnows with training set.'
    ...
    # Test the Winnows with test set
    #print '\n3. Test the Winnows with test set.'
